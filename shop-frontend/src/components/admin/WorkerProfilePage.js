...
import { fetchAppointments, fetchWorkers, bookAppointment } from '../../services/api';
...
function WorkerProfilePage({ onBack }) {
  ...
  // Get worker information from session storage
  ...
  useEffect(() => {
    const loadAppointments = async () => {
      setLoading(true);
      try {
        // In a real implementation, this would filter by the logged-in worker's ID
        const appointmentsData = await fetchAppointments();
        // Filter appointments for just this worker
        const workerAppointments = appointmentsData.filter(appt => 
          appt.worker_id?.toString() === workerId
        );
  ...
  const handleNewAppointment = async () => {
    ...
    // Create appointment data - using the individual worker's ID
    const appointmentData = {
      ...
      phone: phoneField,
      email: emailField,
      // For the individual worker table, we don't need worker_id
      // Instead, we'll use a custom bookWorkerAppointment API function
    };
    
    try {
      // For the prototype, we'll use the standard API call but log that we would use a different table
      console.log('Booking appointment in worker-specific table:', appointmentData);
      
      // In a real implementation, this would call a specific API endpoint for individual worker appointments
      // For now, we'll use the regular function but add worker_id
      ...
      <AppBar position="fixed" color="primary" sx={{ top: 0, left: 0, right: 0 }}>
        <Toolbar>
          ...
          <Typography variant="h6" component="div" sx={{ flexGrow: 1 }}>
            Perfil do Profissional: {workerName}
          </Typography>
        ...
        >
          <Tab label="Meus Agendamentos" icon={<CalendarMonthIcon />} id="worker-tab-0" aria-controls="worker-tabpanel-0" />
          <Tab label="Meu Perfil" icon={<AccountCircleIcon />} id="worker-tab-1" aria-controls="worker-tabpanel-1" />
        </Tabs>
      </AppBar>
            ...
            <Grid container spacing={3}>
              ...
              <Grid item xs={12}>
                <TextField
                  ...
                  fullWidth
                  defaultValue="Serviço 1, Serviço 2"
                  margin="normal"
              ...
              <Grid item xs={12}>
                <TextField
                  ...
                  multiline
                  rows={4}
                  defaultValue="Profissional especializado em diversos serviços. Mais de 5 anos de experiência no setor."
                  margin="normal"
                />
              </Grid>
              <Grid item xs={12}>
