...
export default function Chatbox({ onNewAppointment, workers, freeModeAllowed }) {
  ...
  // Handle date selection - BRUTE FORCE APPROACH
  const handleDateSelect = async (date, addToMessages = true) => {
    if (isGuidedMode()) {
      console.log("🛑 DATE SELECTED - BRUTE FORCE APPROACH");
      console.log(`🛑 Date display: ${date.display}`);
      console.log(`🛑 Date value: ${date.value}`);
      
      // Save current state for context
      const currentWorker = ctx.selectedWorker;
      console.log(`🛑 Current worker: ${currentWorker?.name}`);
      
      // Make a complete copy of the state to modify
      const newState = {
        ...ctx,
      ...
      // Explicitly preserve worker information
      newState.selectedWorker = currentWorker;
      
      console.log("🛑 Setting new booking state with date:", date.value);
      
      // Set the state with our complete object
      ...
      // Add explicit debugging info for the requested time generation
      console.log(`🛑 Will generate times for date ${date.value} and worker ${currentWorker?.name} (ID: ${currentWorker?.worker_id})`)
  ...
  // Handle time selection - COMPLETELY FIXED VERSION
  const handleTimeSelect = (time, addToMessages = true) => {
    if (isGuidedMode()) {
      // CRITICAL: First save all current state to variables before any state updates
      // This prevents race conditions where state might change during the update
      const currentDate = bookingState.selectedDate;
      const currentWorker = bookingState.selectedWorker;
      const currentService = bookingState.selectedService;
      const currentDuration = bookingState.customDuration || 30;
      
      console.log(`🔍 handleTimeSelect called with time: ${time}`);
      console.log(`🔍 Current state BEFORE update:`);
      console.log(`🔍 - Date: ${currentDate}`);
      console.log(`🔍 - Worker: ${currentWorker?.name}`);
      console.log(`🔍 - Service: ${currentService}`);
      
      // CRITICAL FIX: Create a complete state object instead of using functional update
      // This ensures we set all fields explicitly and don't rely on the previous state
      const updatedState = {
        ...bookingState,
        selectedTime: time,
        // EXPLICITLY set these critical fields to ensure they don't change
      ...
      console.log(`🔍 Setting updated state with preserved date: ${updatedState.selectedDate}`);
      
      // Update the booking state with our complete object
      ...
      // Format date for display in dd/mm/yyyy format - using our explicitly saved currentDate
      console.log(`🔍 Formatting confirmation date from saved value: ${currentDate}`);
      ...
      console.log(`🔍 Formatted date for confirmation: ${formattedDate}`);
      
      // Create confirmation message with or without service details - USE OUR SAVED VALUES
      const servicePhrase = currentService === 'Consulta' 
        ? `agendar uma consulta de ${currentDuration} minutos` 
        : `agendar ${currentService}`;
      
      console.log(`🔍 Generating confirmation with:`);
      console.log(`🔍 - Worker: ${currentWorker?.name}`);
      ...
      const confirmQuestion = `Para confirmar, você deseja ${servicePhrase} com ${currentWorker.name} no dia ${formattedDate} às ${time}. Está correto?`;
      ...
      // Use a small timeout to ensure state update happens first
      setTimeout(() => {
        setMessages(prev => [...prev, assistantResponse]);
      }, 50);
    } else {
  ...
  const sendMessage = async (messageText = null) => {
    ...
    if (isGuidedMode()) {
      ...
      if (currentStep === 4 && !availableDates.length) {
      ...
      if (numMatch) {
        ...
        // ——— TIME SELECTION BY NUMBER ———
        if (currentStep === 5 && availableTimes.length >= num && num > 0) {
          // Show the raw input (the number) in the UI
          setMessages(prev => [...prev, { role: 'user', content: textToSend }]);
          
          // Clear input
          setInput('');
          
          // Pick the nth time from the array
          handleTimeSelect(availableTimes[num - 1], false); // false = don't add message
          return;
        }
        
        // Handle service selection by number
        if (currentStep === 2 && commonServices.length >= num && num > 0) {
        ...
        // Handle date selection by number - WITH DEBUGGING
        else if (ctx.step === 4 && availableDates.length >= num && num > 0) {
          const selectedDate = availableDates[num - 1];
          
          // DEBUGGING
          console.log(`🔍 DATE SELECTION BY NUMBER: ${num}`);
          console.log(`🔍 Selected date: ${selectedDate.value} (${selectedDate.display})`);
      ...
      if (ctx.step === 5) {
        ...
        if (enteredText.match(/^\d{1,2}:\d{2}$/)) {
          ...
          const matchedTime = availableTimes.find(t => t.toLowerCase() === enteredText.toLowerCase());
          if (matchedTime) {
            // Show the exact user input in the chat
            setMessages(prev => [...prev, { role: 'user', content: textToSend }]);
            
            setInput('');
            // Pass false since we already added the message
            handleTimeSelect(matchedTime, false);
            return;
          }
        }
        ...
        if (!isValidTime) {
          ...
          // BRUTE FORCE: Get time options using reliable ctx values instead of bookingState
          // This is critical - we use ctx directly which is more reliable
          if (ctx.selectedWorker && ctx.selectedDate) {
            console.log(`🛑 REGENERATING TIMES WITH BRUTE FORCE APPROACH`)
            console.log(`🛑 Using ctx.selectedDate: ${ctx.selectedDate}`)
            console.log(`🛑 Using ctx.selectedWorker: ${ctx.selectedWorker?.name}`)
            
            // Use direct ctx values for reliability
            generateTimeOptions(ctx.selectedWorker.worker_id, ctx.selectedDate)
              .then(() => {
      ...
      // Handle time selection by number - EXTREME BRUTE FORCE APPROACH
      if (numMatch) {
        const num = parseInt(numMatch[0], 10);
        // Only run this code if we're at the time selection step and the number is valid
        if (ctx.step === 5 && availableTimes.length >= num && num > 0) {
          console.log(`🛑 TIME SELECTION NUMBER: ${num} - BRUTE FORCE SOLUTION`)
          
          // Get the selected time directly
          const selectedTime = availableTimes[num - 1];
          console.log(`🛑 Selected time slot #${num}: ${selectedTime}`);
          
          // Show the raw number input in the UI for better UX
          setMessages(prev => [...prev, { role: 'user', content: textToSend }]);
          
          // Clear the input field
          setInput('');
          
          // BRUTE FORCE: Instead of trying to update the existing state or use the handleTimeSelect function,
          // let's bypass all of that and create a completely fresh confirmation message
          
          // First, ensure we get a clean copy of the current state before any updates
          const safeDate = ctx.selectedDate;
          const safeWorker = ctx.selectedWorker;
          const safeService = ctx.selectedService;
          const safeDuration = ctx.customDuration || 30;
          
          console.log(`🛑 Creating confirmation message with these values:`)
          console.log(`🛑 - Date: ${safeDate}`)
          console.log(`🛑 - Worker: ${safeWorker?.name}`)
          ...
          try {
            ...
            formattedDate = safeDate; // Fallback to original format
          }
          
          // Create service phrase for the confirmation
          const servicePhrase = safeService === 'Consulta' 
            ? `agendar uma consulta de ${safeDuration} minutos` 
            : `agendar ${safeService}`;
          
          // Create confirmation message with these reliably stored values
          const confirmQuestion = `Para confirmar, você deseja ${servicePhrase} com ${safeWorker.name} no dia ${formattedDate} às ${selectedTime}. Está correto?`;
          const confirmOptions = "1. Sim\n2. Não";
          ...
          // BYPASS state update chain and just manually update the booking state
          // We're setting all properties directly to avoid any race conditions
          setCtx({
            ...ctx,
            step: 6,
            selectedTime: selectedTime,
            // Explicitly reset these to ensure they don't change
            selectedDate: safeDate,
            selectedWorker: safeWorker,
            selectedService: safeService,
            customDuration: safeDuration,
          });
          
          // Add confirmation message with short delay to ensure state update happens first
          setTimeout(() => {
          ...
          // BRUTE FORCE: Capture state values directly from ctx, not from bookingState
          const safeDate = ctx.selectedDate;
          const safeWorker = ctx.selectedWorker;
          
          console.log(`🛑 INVALID TIME SELECTION - BRUTE FORCE APPROACH`)
          console.log(`🛑 Current state in ctx:`)
          console.log(`🛑 - Date: ${safeDate}`);
          console.log(`🛑 - Worker: ${safeWorker?.name}`);
          
          // Format display date from our safe variables
          let displayDate = '';
          try {
            if (safeDate) {
          ...
          // Generate time options message using our SAFE variables
          if (safeWorker && safeDate) {
            console.log(`🛑 Regenerating time options for date ${safeDate} and worker ${safeWorker?.name}`);
            // Ensure time options are generated for the correct date and worker - using our safe variables
            generateTimeOptions(safeWorker.worker_id, safeDate)
              .then(() => {
                // Get freshly generated time options
                const updatedTimeOptions = formatTimeOptionsMessage();
    ...
    if (isGuidedMode()) {
      ...
      if (context.step === 2) {
        ...
        setTimeout(() => {
          // BRUTE FORCE CRITICAL FIX: Use context values directly instead of bookingState
          // This is crucial because context is the NEW state that was just set
          const workerId = context.selectedWorker?.worker_id;
          const date = context.selectedDate;
          
          console.log(`🛑 FINAL TIME SLOTS REGENERATION - BRUTE FORCE APPROACH`);
          console.log(`🛑 Using CONTEXT values directly:`);
          console.log(`🛑 - Date: ${date}`);
          console.log(`🛑 - Worker ID: ${workerId}`);
          console.log(`🛑 - Worker Name: ${context.selectedWorker?.name}`);
          
          if (workerId && date) {
            console.log(`🛑 Regenerating time options with context values`)
            ...
            console.log(`🛑 Missing worker ID or date in context!`);
