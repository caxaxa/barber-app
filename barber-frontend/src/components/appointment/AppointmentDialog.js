...
export default function AppointmentDialog({
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.date || !formData.start_time || !formData.barber_id) {
      showNotification('Please fill in barber, date, and start time.', 'error');
      return;
    }
    ...
    try {
      ...
      const selectedBarber = barbers.find(b => b.barber_id === formData.barber_id);
      const appointmentWithBarberName = {
        ...formData,
        barber_name: selectedBarber?.name || ''
      };
      
      await bookAppointment(appointmentWithBarberName);
        ...
        <DialogContent dividers sx={{ p: 3 }}>
          ...
          <Box sx={{ mb: 3 }}>
            ...
            <Select
              ...
              inputProps={{ 'aria-label': 'select barber' }}
            >
